...
namespace mes 
{
	...
	{  /*  name          offtype  version   uint8x2         uint8str        string          encstr          uint16x4      deckey  opstrs           */
		...
		{ "dcws0"     , offset1, 0x005C, { 0x00, 0x2B }, { 0xFF, 0xFF }, { 0x2C, 0x45 }, { 0x46, 0x49 }, { 0x4A, 0xFF },  0x20, { 0x42, 0x48 }/*---*/}, // D.C. White Season ~ダ・カーポ ホワイトシーズン~ クリスマス限定版
		{ "dcws"      , offset1, 0x656C, { 0x00, 0x2B }, { 0x2C, 0x31 }, { 0x32, 0x4C }, { 0x4D, 0x50 }, { 0x51, 0xFF },  0x20, { 0x42, 0x48 }/*---*/}, // D.C. White Season（D.C.～ダ・カーポ～アーカイブス　SAKURA Edition）
		...
		{ "dcdx"      , offset1, 0x7769, { 0x00, 0x28 }, { 0x29, 0x2E }, { 0x2F, 0x49 }, { 0x4A, 0x4D }, { 0x4E, 0xFF },  0x20, { 0x45 }/*---------*/}, // D.C.Dream X’mas～ダ・カーポ～ドリームクリスマス
		{ "dcas"      , offset1, 0x4E69, { 0x00, 0x28 }, { 0x29, 0x2E }, { 0x2F, 0x49 }, { 0x4A, 0x4D }, { 0x4E, 0xFF },  0x20, { 0x43 }/*---------*/}, // D.C.A.S. 〜ダ・カーポ〜アフターシーズンズ
		{ "dcbs"      , offset1, 0x3163, { 0x00, 0x2B }, { 0xFF, 0xFF }, { 0x2C, 0x48 }, { 0x49, 0x4C }, { 0x4D, 0xFF },  0x20, {/*——*/}/*---------*/}, // D.C.II ～ダ・カーポII～ 春風のアルティメットバトル!
		{ "dc2fl"     , offset1, 0x9C69, { 0x00, 0x28 }, { 0x29, 0x2E }, { 0x2F, 0x49 }, { 0x4A, 0x4D }, { 0x4E, 0xFF },  0x20, { 0x45 }/*---------*/}, // D.C.II Fall in Love ～ダ・カーポII～フォーリンラブ
		{ "dc2bs"     , offset1, 0x316C, { 0x00, 0x2B }, { 0x2C, 0x31 }, { 0x32, 0x4C }, { 0x4D, 0x50 }, { 0x51, 0xFF },  0x20, {/*——*/}/*---------*/}, // D.C.II ～ダ・カーポII～ 春風のアルティメットバトル!
		{ "dc2dm"     , offset1, 0x9D72, { 0x00, 0x29 }, { 0x2A, 0x31 }, { 0x32, 0x4C }, { 0x4D, 0x50 }, { 0x51, 0xFF },  0x20, { 0x44 }/*---------*/}, // D.C.II Dearest Marriage ～ダ・カーポII～ ディアレストマリッジ
		...
		{ "dc2ty"     , offset1, 0x5F69, { 0x00, 0x28 }, { 0x29, 0x2E }, { 0x2F, 0x49 }, { 0x4A, 0x4D }, { 0x4E, 0xFF },  0x20, {/*——*/}/*---------*/}, // D.C.II To You ～ダ・カーポII～トゥーユー サイドエピソード
		...
		{ "puripa"    , offset1, 0x5B6C, { 0x00, 0x28 }, { 0x29, 0x2F }, { 0x30, 0x4A }, { 0x4B, 0x4E }, { 0x4F, 0xFF },  0x20, { 0x46 }/*---------*/}, // Princess Party ～プリンセスパーティー～
		{ "uni"       , offset1, 0x746A, { 0x00, 0x29 }, { 0x2A, 0x2F }, { 0x30, 0x4A }, { 0x4B, 0x4E }, { 0x4F, 0xFF },  0x20, { 0x46 }/*---------*/}, // ユニ uni.
		{ "homemaid"  , offset1, 0x1A63, { 0x00, 0x2B }, { 0xFF, 0xFF }, { 0x2C, 0x48 }, { 0x49, 0x4C }, { 0x4D, 0xFF },  0x20, {/*——*/}/*---------*/}, // ホームメイド -Home maid-
		{ "hmsw"      , offset1, 0x4F69, { 0x00, 0x28 }, { 0x29, 0x2E }, { 0x2F, 0x49 }, { 0x4A, 0x4D }, { 0x4E, 0xFF },  0x20, {/*——*/}/*---------*/}, // ホームメイド スイーツ
		{ "ag2dc"     , offset1, 0x7C6A, { 0x00, 0x29 }, { 0x2A, 0x2F }, { 0x30, 0x4A }, { 0x4B, 0x4E }, { 0x4F, 0xFF },  0x20, { 0x46 }/*---------*/}, // A.G.II.D.C.　～あるぴじ学園2.0　サーカス史上最大の危機！？～
		{ "suika"     , offset1, 0x005F, { 0x00, 0x2B }, { 0xFF, 0xFF }, { 0x2C, 0x46 }, { 0x47, 0x4A }, { 0x4B, 0xFF },  0x20, { 0x42 }/*---------*/}, // 水夏 ~SUIKA~全年齢版、水夏 ~SUIKAおー・157章~
	...
	auto script_helper::fetch_scene_text(bool absolute_file_offset) const -> std::vector<script_helper::text> 
	{
		...
		for (const script_view::token& token : this->m_mesview.tokens()) 
		{
			if (info->string.its(token.value))
			{
				...
				if (static_cast<uint8_t>(text[0]) >= 125)
				{
